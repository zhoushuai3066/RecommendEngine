 ...
 */
public final class ZhouModel implements DataModel {
...
private static final Logger log = LoggerFactory.getLogger(ZhouModel.class);
   ...
   */
  public ZhouModel() throws UnknownHostException {
  ...
  private void buildModel() throws UnknownHostException {
    ...
    if (!mongoAuth || db.authenticate(mongoUsername, mongoPassword.toCharArray())) {
      ...
      while (cursor.hasNext()) {
        Map<String,Object> user = (Map<String,Object>) cursor.next().toMap();
        if (!user.containsKey("deleted_at")) {
//        	System.out.println(getID(user.get(mongoUserID),true));
//        	System.out.println(getID(user.get(mongoItemID),true));
